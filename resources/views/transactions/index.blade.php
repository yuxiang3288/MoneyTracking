...
@section('content')
    ...
    <div class="flex flex-col lg:flex-row gap-6">
        <!-- Filter Sidebar -->
        <aside class="lg:w-1/5 w-full bg-white rounded shadow p-4">
            ...
            <form method="GET" action="{{ route('transactions.index') }}" class="space-y-4">
                ...
                <button type="submit" class="w-full bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Apply
                    Filters</button>
            </form>
        </aside>

        <!-- Transactions Table -->
        <div class="lg:w-3/5 w-full bg-white rounded shadow p-4">
            <!-- Month navigation -->
            <div class="flex items-center justify-between mb-4 w-full space-x-4">
                <a href="{{ route('transactions.index', ['month' => $prevMonth->format('Y-m')]) }}"
                    class="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300">
                    < </a>

                        <h2 class="text-lg font-semibold text-center flex-1">{{ $currentDate->format('F Y') }}</h2>

                        <a href="{{ route('transactions.index', ['month' => $nextMonth->format('Y-m')]) }}"
                            class="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300">>
                        </a>
            </div>
            ...
            <div x-show="showModal" x-cloak
                ...
                <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md relative">
                    ...
                    <button @click="showModal = false"
                        class="absolute top-2 right-2 text-gray-500 text-xl hover:text-red-600">
                        &times;
                    </button>
